...
// Called by NMEA2000 library when CAN bus communication starts
void OnN2kOpen() {
}
...
void canDashboardNMEA2000(CanCycle cycle) {
	...
	if (cycle.isInterval(CI::_100ms))
	{
		...
		float lambda1raw = Sensor::getOrZero(SensorType::Lambda1) / 0.0001f;
		float lambda2raw = Sensor::getOrZero(SensorType::Lambda2) / 0.0001f;
		uint16_t lambdaVal[2u] = {
			(uint16_t)clampF(0, lambda1raw, 65535),
			(uint16_t)clampF(0, lambda2raw, 65535) };
		...
		{
			if (lambdaVal[i] != 0 && lambdaVal[i] > outputLambda)
			{
				outputLambda = lambdaVal[i];
			}
		}
	...
	if (cycle.isInterval(CI::_1000ms))
	{
		...
		/* Low voltage — RPM-dependent thresholds with 5s debounce for trim pump / anchor winch */
		...
		flagLowSystemVoltage = battVoltage < battVoltageThreshold;
